...
import { ProjectSubmission, EventSubmission, SubmissionType, ChecklistItem, Reminder } from '../types/submissions';
import { sendEmail, formatSubmissionNotificationEmail } from '../utils/emailService';
  ...
  const handleSubmit = async (status: 'draft' | 'pending') => {
    ...
    try {
      const baseSubmission = {
        ...
        auditTrail: [{
          action: status === 'draft' ? 'Created as draft' : 'Submitted for review',
          performedBy: currentUser.uid,
          performedAt: serverTimestamp(),
          details: `${submissionType} ${status === 'draft' ? 'saved as draft' : 'submitted for admin review'}`
        }]
      };

      let submission: ProjectSubmission | EventSubmission;
      ...
      if (submissionType === 'project') {
        ...
        collectionName = 'projectSubmissions';
      } else {
        submission = {
          ...eventData,
          ...baseSubmission,
        ...
        collectionName = 'eventSubmissions';
      }
      ...
      if (status === 'pending') {
        await sendEmail(formatSubmissionNotificationEmail({
          type: submissionType,
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-luxury-display text-black mb-4">
            Create New {submissionType === 'project' ? 'Project' : 'Event'}
          </h1>
          <p className="text-xl text-black font-luxury-body">
            Submit your {submissionType} idea for review and approval by our team.
          </p>
        </div>
